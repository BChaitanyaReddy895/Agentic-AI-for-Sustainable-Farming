...
def calculate_sustainability_score(row):
    # Score out of 100
    score = 100
    # Water usage penalty (linear, 0 penalty if <= recommended, up to -30 if double recommended)
    water = row.get('water_score', 0)
    if water > RECOMMENDED_WATER:
        score -= min(30, 30 * (water - RECOMMENDED_WATER) / RECOMMENDED_WATER)
    # Fertilizer usage penalty (linear, 0 penalty if <= recommended, up to -30 if double recommended)
